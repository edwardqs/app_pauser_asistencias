...
DROP FUNCTION IF EXISTS public.register_manual_attendance(uuid, uuid, date, timestamptz, text, text, text, text, boolean);
DROP FUNCTION IF EXISTS public.register_manual_attendance(uuid, uuid, date, timestamptz, text, text, text, text);
DROP FUNCTION IF EXISTS public.register_manual_attendance(uuid, uuid, date, timestamp with time zone, text, text, text, text, boolean, jsonb);
DROP FUNCTION IF EXISTS public.register_manual_attendance(uuid, uuid, date, timestamp with time zone, text, text, text, text, boolean); -- Variante timestamp sin location
DROP FUNCTION IF EXISTS public.register_manual_attendance(uuid, uuid, date, timestamp with time zone, timestamp with time zone, text, text, text, text, boolean, jsonb); -- Variante con check_out explicito
DROP FUNCTION IF EXISTS public.register_manual_attendance(uuid, uuid, date, timestamptz, timestamptz, text, text, text, text, boolean, jsonb); -- Variante con check_out explicito (timestamptz alias)
...
CREATE OR REPLACE FUNCTION public.register_manual_attendance(
    ...
    p_check_out timestamptz,    -- NUEVO PARAMETRO
...
BEGIN
  ...
  IF p_record_type = 'ASISTENCIA' OR p_record_type = 'IN' THEN
     ...
     -- v_check_in ya asignado arriba
     
     -- Lógica de Tardanza:
     -- Si el usuario marcó check "Tarde" en la app (p_is_late) O si la hora supera el límite.
     IF p_is_late OR (v_time_peru IS NOT NULL AND v_time_peru > c_late_limit) THEN
        v_is_late := true;
        v_status := 'tardanza';
     ELSE
        v_is_late := false;
  ...
  ELSE
     ...
     v_check_in := NULL; 
     v_check_out := NULL; -- También limpiamos salida por si acaso
  END IF;
  ...
  IF v_existing IS NOT NULL THEN
    ...
    SET 
      ...
      check_out = COALESCE(v_check_out, check_out), -- Actualizamos salida si se envió
    ...
    ELSE
        -- Si es asistencia, aseguramos que se actualice el check_in y check_out si se enviaron
        -- (El UPDATE de arriba ya usa COALESCE, pero si v_check_in es NULL y queríamos borrarlo, COALESCE no sirve)
        -- En modo MANUAL, si mandamos NULL es porque queremos que sea NULL? No, p_check_in es requerido para asistencia.
        NULL; -- No-op
    END IF;
