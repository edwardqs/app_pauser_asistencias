-- =============================================================================
-- PARCHE ROBUSTO: Limpieza Total y Despliegue de register_attendance V5
-- =============================================================================
-- EJECUTAR ESTE SCRIPT COMPLETO EN EL SQL EDITOR DE SUPABASE
-- =============================================================================

-- 1. LIMPIAR ABSOLUTAMENTE TODAS LAS VERSIONES SOBRECARGADAS DE LA FUNCIÓN
-- Esto previene colisiones en la API PostgREST y asegura que se ejecute la lógica correcta.
DROP FUNCTION IF EXISTS public.register_attendance(uuid, double precision, double precision, text);
DROP FUNCTION IF EXISTS public.register_attendance(uuid, double precision, double precision, text, text);
...
DROP FUNCTION IF EXISTS public.register_attendance(uuid, double precision, double precision, text, text, text, text, text);
DROP FUNCTION IF EXISTS public.register_attendance(uuid, text, double precision, double precision, text, text, text);
DROP FUNCTION IF EXISTS public.register_attendance(uuid, double precision, double precision, text, text, text, text, text, text);


-- 2. CREAR LA FUNCIÓN REGISTER_ATTENDANCE V5 (ROBUSTA Y CON FALLBACK POR PROXIMIDAD)
...
BEGIN
    ...
    -- Paso 1: Intentar buscar coincidencia exacta por turno (p_shift)
    SELECT ws.*
    INTO v_schedule
    ...
    -- Paso 2: Si no hay coincidencia exacta del turno (por ejemplo, cuando la app manda 'UNICO' pero el horario es 'MAÑANA'),
    -- se busca cualquier horario activo para hoy, priorizando los especiales y luego por proximidad horaria al marcado.
    IF v_schedule IS NULL THEN
        ...
        ORDER BY
          CASE WHEN ws.schedule_type != 'REGULAR' THEN 0 ELSE 1 END,
          ABS(EXTRACT(EPOCH FROM (v_now_peru::time - ws.check_in_time))) ASC,
          esa.valid_from DESC
        LIMIT 1;
    END IF;

    -- Valores con fallback robusto
    ...
    IF p_type = 'IN' THEN
        ...
        INSERT INTO public.attendance (
            ...
            p_evidence_url, 'ASISTENCIA', v_schedule_id, v_shift_to_save, v_now_utc
     ...
     ELSE
         ...
         BEGIN
             INSERT INTO public.attendance (
                 ...
                 v_final_type, v_schedule_id, v_shift_to_save, v_now_utc,
                 jsonb_build_object('lat', p_lat, 'lng', p_lng)
             ) RETURNING id INTO v_attendance_id;
...
-- 3. VOLVER A OTORGAR PERMISOS A LA FIRMA CORRECTA
...
GRANT EXECUTE ON FUNCTION public.register_attendance(uuid, double precision, double precision, text, text, text, text) TO service_role;
